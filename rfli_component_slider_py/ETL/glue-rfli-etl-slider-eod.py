...
def send_error_mail(timeout=False):
    try:
        ...
        subject = f"[SLIDER_EOD] Error intradia Slider{report_date.strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        if timeout is True:
            subject = f"[SLIDER_EOD] TIMEOUT_ERROR intradia Slider {report_date.strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    def update_last_version_info(self):
        try:
            ...
            if 'Item' in response:
                ...
                print("No se encontró información de versión para 'slider'. No se cambia el versionado.")
