...
def send_error_mail(timeout=False):
    try:
        ...
        subject = "[ISIN_TRACK_EOD] Error intradia seguimiento de isines {}".format(
...
class EodIsinTrack:
    ...
    def run(self):
        ...
        except Exception as eod_compare_curves_exception:
            exception_line = sys_exc_info()[2].tb_lineno
            logger.error(
                "Ocurrió un error inseperado en la actualización de la data.")
            current_error = eod_compare_curves_exception
            logger.error(current_error.__class__.__name__ +
                         "[" + str(exception_line) + "] " + str(current_error))
            send_error_mail()
        ...
        for current_isin in today_isines:
            ...
            if (current_isin_code in today_category_temp_dictionary):
                ...
                new_isin_object = {
                    ...
                    "rating": current_isin["real_rating"]
                    }
                ...
                with connection.cursor() as cursor_connection:
                    try:
                        ...
                        if cursor_connection.rowcount > 0:
                            ...
                            isines = self.merge_all_isines_data(
    ...
    def update_last_version_info(self):
        try:
            ...
            if 'Item' in response:
                ...
                print("No se encontró información de versión para 'isin-track'. No se cambia el versionado.")
                ...
                final_isines_data.append(
                    {
                        ...
                        "data": temp_isin_dictionary["data"].copy(),
                    }
                )
