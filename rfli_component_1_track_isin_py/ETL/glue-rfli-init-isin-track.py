...
def send_error_mail(timeout=False):
    try:
        ...
        subject = "[INTRADAY_RESET_DATA_ISIN_TRACK] Error intradia limpiando tablas Isin Track {}".format(report_date.strftime('%Y-%m-%d %H:%M:%S'))
        body = f"Se ha generado un error al limpiar las tablas para isin_track. Por favor informar para su respectiva revision.\n\
            Mensaje enviado por servicio ETL Intradia a la fecha y hora {report_date.strftime('%Y-%m-%d %H:%M:%S')}"
        if timeout is True:
            subject = "[INTRADAY_RESET_DATA_ISIN_TRACK] TIMEOUT_ERROR Error intradia limpiando tablas Isin Track {}".format(
            report_date.strftime('%Y-%m-%d %H:%M:%S'))
            body = f"Se ha generado un error al limpiar las tablas para isin_track. El error ha sido generado\
...
class InitIsinTrack:
    ...
    def __init__(self):
        ...
        self.dynamo_db_collections_intra = [
            ...
            "dnb-rfli-isin-track-folios"
        ]
        self.dynamodb_session = bt3_resource('dynamodb')
        self.inactive_isines = self.get_yesterday_inactive_isnes()
    ...
    def get_yesterday_inactive_isnes(self):
        ...
        get_inactive_isines_query = f"SELECT {self.create_string_query(self.query_timeout)} \
                ...
                WHERE\
                    valuation_date = '{yesterday_full_valuation_date.date().strftime('%Y-%m-%d')}'\
                    AND maturity_date = '{full_valuation_date.date().strftime('%Y-%m-%d')}'\
                    AND isin_code != '';"
        try:
        ...
        try:
            ...
            with table_to_empty.batch_writer() as batch:
                for key in all_table_items:
                    if (table_name=='dnb-rfli-isin-track-all-isines' or table_name=='dnb-rfli-isin-track-folios'
