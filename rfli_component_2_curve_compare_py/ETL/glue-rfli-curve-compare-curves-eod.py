...
def send_error_mail():
    try:
        ...
        subject = "[COMPARE_CURVES_EOD] Error intradia Curvas y Folios {}".format(report_date.strftime('%Y-%m-%d %H:%M:%S'))
        body = f"Se ha generado un error al actualizar la información de curvas y folios en la tabla de historicos(EOD). Por favor informar para su respectiva revision.\n\
...
class IntradayCompareCurves:
    ...
    def __init__(self):
        self.full_valuation_date = dt.datetime.strptime(get_enviroment_variable('VALUATION_DATE'),'%Y-%m-%d')
    ...
    def run(self):
        try:
            ...
            if bogota_current_datetime.timestamp()>=prelim_start_timestamp:
                ...
                logger.info("No ha empezado el periodo preliminar. No se actualiza la data.")
    ...
    def get_origin_data(self):
        ...
        try:
            with pymysql.connect(host=get_secret(get_enviroment_variable('FLASH_ORIGIN_DB'))['host'],
                                        port=int(get_secret(get_enviroment_variable('FLASH_ORIGIN_DB'))['port']),
                                        user=get_secret(get_enviroment_variable('FLASH_ORIGIN_DB'))['username'],
                                        password=get_secret(get_enviroment_variable('FLASH_ORIGIN_DB'))['password'],
                    ...
                    try:
                        ...
                        if cursor_connection.rowcount > 0:
                            ...
                            if cursor_connection.rowcount > 0:
                                ...
                                logger.error("No se encontró información de los folios.")
                        else:
        ...
        if curves_data:
            for i in range(5):
                transformed_curves_data.append(
                    {
                        'valuation_date': self.full_valuation_date.strftime('%Y-%m-%d'),
                ...
                if i <= 1:
                    transformed_curves_data[-1]['data']['beta_0_r'] = curves_data[3]['beta_0']
                    transformed_curves_data[-1]['data']['beta_1_r'] = curves_data[3]['beta_1']
                    transformed_curves_data[-1]['data']['beta_2_r'] = curves_data[3]['beta_2']
                    transformed_curves_data[-1]['data']['tau_r'] = curves_data[3]['tau']
                elif i == 2:
                    transformed_curves_data[-1]['data']['beta_0_r'] = curves_data[4]['beta_0']
                    transformed_curves_data[-1]['data']['beta_1_r'] = curves_data[4]['beta_1']
                    transformed_curves_data[-1]['data']['beta_2_r'] = curves_data[4]['beta_2']
        ...
        if folios_data:
            ...
            for key in initial_dictionary.keys():
                ...
                transformed_folios_data.append(
                    {
                        'valuation_date': self.full_valuation_date.strftime('%Y-%m-%d'),#str(get_enviroment_variable('VALUATION_DATE')),
                        'cc_curve': key,
                        'data': initial_dictionary[key].copy()
                    }
                )
        else:
    ...
    def update_last_version_info(self):
        try:
            ...
            if 'Item' in response:
                ...
                print("No se encontró información de versión para 'compare-curve'. No se cambia el versionado.")
