...
def lambda_handler(event, context):
    try:
        ...
        logger.info(f"Se lanzó el Glue - Top Delta Category EOD con el id {job_run_id}.")
        return {
            'statusCode': 200,
            'body': json.dumps('Se lanzaron el/los Glue correctamente')
        }
    except Exception:
        logger.error("Error iniciando el Glue para componente fin de dia - Top Delta Category")
        raise
