...
class LookUp:
    def __init__(self):
        ...
        self.full_valuation_date = get_bogota_current_time().replace(microsecond=0)
        self.full_valuation_date = dt.datetime.strptime(
            f"{self.full_valuation_date}", "%Y-%m-%d %H:%M:%S"
        )
    ...
    def update_last_version_info(self):
        ...
        if "Item" in response:
            ...
            print(
                "No se encontró información de versión para 'isin-search'. No se cambia el versionado."
            )
    ...
    def run(self):
        try:
            ...
            self.save_data_into_dynamo("dnb-rfli-isin-search-all-isines", data)
