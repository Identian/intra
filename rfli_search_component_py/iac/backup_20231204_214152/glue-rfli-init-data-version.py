...
class InitDataVersion:
    def __init__(self):
        ...
        self.dynamo_db_collections_intra = [
            "dnb-rfli-curve-compare-curves-intra",
            "dnb-rfli-curve-compare-folios-intra"
        ]
            ...
            with pymysql.connect(host=get_secret(get_enviroment_variable('FLASH_ORIGIN_DB'))['host'],
                ...
                with connection.cursor() as cursor_connection:
                    try:
                        ...
                        business_day_consult = f"SELECT business_date\
    ...
    def empty_table_data(self, table_name):
        try:
            ...
            scan = dynamodb_client.scan(TableName=table_name)
            items = scan['Items']
            delete_requests = []
            for item in items:
                delete_key = {}
                delete_key[primary_key] = item[primary_key]
                if sort_key:
            ...
            logger.info("Se inicia el proceso de eliminación de items")
            for i in range(0, len(delete_requests), 25):
                batch = delete_requests[i:i+25]
                request = {'RequestItems': {table_name: batch}}
                response = dynamodb_client.batch_write_item(**request)
                print(f"Resultado de eliminación en lote: {response}")
        except Exception as insert_in_destination_exception:
            exception_line = sys_exc_info()[2].tb_lineno
            logger.error(
    ...
    def reset_versions(self):
        try:
            ...
            current_day = get_bogota_current_time().replace(hour=5,minute=0,second=0,microsecond=0)
