...
def send_error_mail(timeout=False):
    try:
        ...
        subject = "[INTRADAY_RESET_DATA_PORTFOLIO_TRACK] Error intradia limpiando tablas PORTFOLIO TRACK {}".format(report_date.strftime('%Y-%m-%d %H:%M:%S'))
        body = f"Se ha generado un error al limpiar las tablas para PORTFOLIO TRACK. Por favor informar para su respectiva revision.\n\
            Mensaje enviado por servicio ETL Intradia a la fecha y hora {report_date.strftime('%Y-%m-%d %H:%M:%S')}"
        if timeout is True:
            subject = "[INTRADAY_RESET_DATA_PORTFOLIO_TRACK] TIMEOUT_ERROR Error intradia limpiando tablas Isin Track {}".format(
            report_date.strftime('%Y-%m-%d %H:%M:%S'))
            body = f"Se ha generado un error al limpiar las tablas para PORTFOLIO TRACK. El error ha sido generado\
        ...
        try:
            ...
            with table_to_empty.batch_writer() as batch:
                for key in all_table_items:
                    if (table_name=='dnb-rfli-portfolio-track-all-isines'
